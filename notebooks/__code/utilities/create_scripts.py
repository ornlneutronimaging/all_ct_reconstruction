...
def create_sh_file(json_file_name: str, output_folder: str) -> str:
    ...
    with open(sh_file_name, 'w') as sh_file:
        ...
        #sh_file.write(f"~/.pixi/bin/pixi run --manifest-path /SNS/VENUS/shared/software/git/all_ct_reconstruction python {STEP3_SCRIPTS} {json_file_name_on_linux}\n")
        sh_file.write(f"~/.pixi/bin/pixi run --manifest-path ~/notebooks/all_ct_reconstruction python {STEP3_SCRIPTS} {json_file_name_on_linux}\n")
